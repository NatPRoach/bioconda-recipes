...
cp ReleaseMT/bin/igblastn $SHARE_BIN
cp ReleaseMT/bin/igblastp $SHARE_BIN
cp ReleaseMT/bin/makeblastdb $PREFIX/bin/makeigblastdb
cp -R src/app/igblast/internal_data $SHARE_DIR
cp -R src/app/igblast/optional_file $SHARE_DIR

# Replace the shebang
sed '1 s_^.*$_#!/usr/bin/env perl_' ReleaseMT/bin/edit_imgt_file.pl > $PREFIX/bin/edit_imgt_file.pl
